...
public abstract class TrailRenderer_Base : MonoBehaviour
{
	...
	protected virtual void Update()
	{
		...
		{
			UpdatePoints(_activeTrail, TimeManager.Instance.m_NormalDelta);
			UpdateTrail(_activeTrail, TimeManager.Instance.m_NormalDelta);
		...
		for (int num = _fadingTrails.Count - 1; num >= 0; num--)
		{
			...
			{
				UpdatePoints(_fadingTrails[num], TimeManager.Instance.m_NormalDelta);
				UpdateTrail(_fadingTrails[num], TimeManager.Instance.m_NormalDelta);
