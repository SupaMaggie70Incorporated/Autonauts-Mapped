...
[MoonSharpUserData]
public class ModFood : ModCustom
{
	...
	{
		return "WorldObjects/Special/FoodMod";
	}

	public override ObjectSubCategory GetSubcategory()
	{
		return ObjectSubCategory.FoodOther;
	}
	...
	public void CreateFood(string UniqueName, string[] NewIngredientsStringArr = null, int[] NewIngredientsAmountArr = null, string ModelName = "", bool UsingCustomModel = true)
	{
		if (UniqueName.Length == 0)
		{
			string descriptionOverride = "Error: ModFood.CreateFood '" + UniqueName + "' - Unique Name is null length";
		...
		{
			string descriptionOverride2 = "Error: ModFood.CreateFood '" + UniqueName + "' - Ingredients and Ingredient amounts not equal";
		...
		if (!GeneralUtils.m_InGame)
		{
			...
			{
				string descriptionOverride3 = "Error: ModFood.CreateFood '" + UniqueName + "' - already used this name!";
			...
			if (DebugInfo)
			{
				Debug.Log("ADDED NEW FOOD CALLED " + UniqueName + " (" + UniqueName + ")  ObjID " + objectType);
			}
			...
			string descriptionOverride4 = "Error: ModFood.CreateFood - Cannot find Lua Script";
		...
		if (!HasSetIngredients[modObjectTypeFromName] && NewIngredientsStringArr != null)
		{
			...
			for (int i = 0; i < NewIngredientsStringArr.Length; i++)
			{
				...
				if (!Enum.TryParse<ObjectType>(NewIngredientsStringArr[i], out result))
				{
					...
					{
						string descriptionOverride5 = "Error: ModFood.CreateFood - Object Ingredient '" + NewIngredientsStringArr[i] + "' - cannot be found";
