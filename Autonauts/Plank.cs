...
public class Plank : Holdable
{
	...
	private void EndAxe(AFO Info)
	{
		for (int i = 0; i < 2; i++)
		{
			BaseClass baseClass = ObjectTypeList.Instance.CreateObjectFromIdentifier(ObjectType.Pole, base.transform.position, base.transform.localRotation);
			SpawnAnimationManager.Instance.AddJump(baseClass, m_TileCoord, m_TileCoord, 0f, baseClass.transform.position.y, 3f);
			QuestManager.Instance.AddEvent(QuestEvent.Type.Make, Info.m_Actioner.m_TypeIdentifier == ObjectType.Worker, ObjectType.Pole, Info.m_Actioner);
		}
