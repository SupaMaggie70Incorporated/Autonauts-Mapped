...
public class ModTop : ModCustom
{
	...
	{
		return "WorldObjects/Clothes/Tops/TopMod";
	}

	public override ObjectSubCategory GetSubcategory()
	{
		return ObjectSubCategory.ClothingCotton;
	}
	...
	public void CreateTop(string UniqueName, string[] NewIngredientsStringArr = null, int[] NewIngredientsAmountArr = null, string ModelName = "", bool UsingCustomModel = true)
	{
		if (UniqueName.Length == 0)
		{
			string descriptionOverride = "Error: ModTop.CreateTop '" + UniqueName + "' - Unique Name is null length";
		...
		{
			string descriptionOverride2 = "Error: ModTop.CreateTop '" + UniqueName + "' - Ingredients and Ingredient amounts not equal";
		...
		if (!GeneralUtils.m_InGame)
		{
			...
			{
				string descriptionOverride3 = "Error: ModTop.CreateTop '" + UniqueName + "' - already used this name!";
			...
			if (ModelName.Length == 0)
			{
				ModelName = "Models/Clothes/Tops/TopLumberjack";
			}
			...
			if (DebugInfo)
			{
				Debug.Log("ADDED NEW TOP CALLED " + UniqueName + " (" + UniqueName + ")  ObjID " + objectType);
			}
			...
			string descriptionOverride4 = "Error: ModTop.CreateTop - Cannot find Lua Script";
		...
		if (!HasSetIngredients[modObjectTypeFromName] && NewIngredientsStringArr != null)
		{
			...
			for (int i = 0; i < NewIngredientsStringArr.Length; i++)
			{
				...
				if (!Enum.TryParse<ObjectType>(NewIngredientsStringArr[i], out result))
				{
					...
					{
						string descriptionOverride5 = "Error: ModTop.CreateTop - Object Ingredient '" + NewIngredientsStringArr[i] + "' - cannot be found";
