...
public class Pole : Holdable
{
	...
	private void EndAxe(AFO Info)
	{
		for (int i = 0; i < 1; i++)
		{
			BaseClass baseClass = ObjectTypeList.Instance.CreateObjectFromIdentifier(ObjectType.FixingPeg, base.transform.position, base.transform.localRotation);
		...
		QuestManager.Instance.AddEvent(QuestEvent.Type.Make, Info.m_Actioner.m_TypeIdentifier == ObjectType.Worker, ObjectType.FixingPeg, this);
		StopUsing();
	}
	...
	private ActionType GetActionFromAxe(AFO Info)
	{
		...
		if (QuestManager.Instance.GetIsObjectLocked(ObjectType.FixingPeg))
		{
